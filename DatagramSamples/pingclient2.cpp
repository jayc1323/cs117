...
main(int argc, char *argv[]) {
    ...
    try {
        ...
        sock -> turnOnTimeouts(3000); // turn on timeouts - 3 seconds for every read on this socket
        ...
        c150debug->printf(C150APPLICATION,"%s: Writing message: \"%s\"",
                          argv[0], message);
        sock -> write(message.c_str(),message.length()+1); // +1 includes the null
        ...
        /* Retry block in case there was a timeout on the first read */
        //-------------------------------------------------------------------------
        timedOut = sock->timedout();
        int retries = 0;
        while(timedOut && retries<=5){
      
            retries++;
            c150debug->printf(C150APPLICATION,"%s: Retry no %d Resending message: \"%s\"",
                          argv[0],retries, message);
            sock -> write(message.c_str(),message.length()+1); // +1 includes the null
            readlen = sock -> read(incomingMessage, sizeof(incomingMessage));
            timedOut = sock->timedout();
          
        }
        //If timed out even after 5 retries
        if (timedOut){
            throw C150NetworkException("Server is not responding,max retries reached");
        }
        //--------------------------------------------------------------------------

        // Check and print the first incoming message
        checkAndPrintMessage(readlen, incomingMessage, sizeof(incomingMessage));

        //Loop reading and printing until timeout occurs
        bool subsequentReadTimedOut = false;
        while(!subsequentReadTimedOut){
             readlen = sock -> read(incomingMessage, sizeof(incomingMessage));
             subsequentReadTimedOut = sock->timedout();
             if(subsequentReadTimedOut) break;
